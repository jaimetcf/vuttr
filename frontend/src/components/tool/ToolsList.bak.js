...
const ToolsList = props => {
  ...
  // from the database thah have the specified tag, if any.
  ...
  const removeTool = async (id) => {
    ...
    fetch( appContext.backendDomain + '/tools/' + id, {
        ...
        if(res.ok) {   // Status = 2xx
          setWaiting(false);   // Hides waiting spinner
          getUserAllTools();   // Reloads all database tools for current user
        }
        else {         // Status IS NOT 2xx
      ...
      {!removeId && !waiting && (
        <div>
          ...
          <Panel>
            ...
            <Button size='small' to={'/tools/add'} float='right'>+ Add</Button>
          </Panel>
          {(tools.length>0) && (
